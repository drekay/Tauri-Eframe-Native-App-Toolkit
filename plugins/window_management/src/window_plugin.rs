...
//plugins/window_management/window_plugin.rs
...
use crate::{ui::egui_impl::components::Window, Message, Plugin, UIPlugin, WindowPluginMessage};
const FRAME_HEIGHT: f32 = 100.0;


impl Plugin for WindowPlugin {
   /*  fn handle_messageold(&self, message: Message) {
...
impl UIPlugin for WindowPlugin {
...
impl WindowPlugin {
   ...
   /* short - fn handle_message(&mut self, message: Message, tx: &Sender<Message>) {
        match message {
            Message::WindowPlugin(window_message) => {
                match window_message {
                    WindowPluginMessage::AddWindow => {
                        ...
                        tx.send(Message::WindowControllerPlugin(WindowControllerPluginMessage::WindowAdded)).unwrap();
                    },
                    WindowPluginMessage::CollapseWindow(id) => {
                        // Handle collapsing a window
                        println!("Collapsing window with id: {}", id);
                    },
                    // Handle other WindowPluginMessage variants...
                }
            },
        ...
        Message::WindowPlugin(window_message) => {
            match window_message {
                WindowPluginMessage::AddWindow => {
                    ...
                    // You might want to send a message back to confirm the window was added
                   //Todo: tx.send(Message::WindowControllerPlugin(WindowControllerPluginMessage::WindowAdded)).unwrap();
                },
                WindowPluginMessage::CollapseWindow(id) => {
                    // Handle collapsing a window
                    println!("Collapsing window with id: {}", id);
                },
                WindowPluginMessage::DragWindowStart(id, pos) => {
                    // Handle start of window drag
                    println!("Started dragging window {} at position {:?}", id, pos);
                },
                WindowPluginMessage::DragWindowMove(pos) => {
                    // Handle window drag movement
                    println!("Dragging window to position {:?}", pos);
                },
                WindowPluginMessage::DragWindowEnd => {
                    // Handle end of window drag
                    println!("Finished dragging window");
                },
                WindowPluginMessage::MinimizeWindow(id) => {
                    // Handle minimizing a window
                    println!("Minimizing window with id: {}", id);
                },
                WindowPluginMessage::ConfirmedCloseWindow(id) => {
                    // Handle confirmation of window closure
                    println!("Confirmed closure of window with id: {}", id);
                },
            }
        },
        Message::WindowControllerPlugin(_) => {
            // Handle any WindowControllerPlugin messages if needed
        },
        Message::Broadcast(content) => {
            // Handle broadcast messages
            println!("Received broadcast: {}", content);
        },
...
//RENDER CODE
   ...
   /*  fn add_window(&mut self) {
    ...
    fn add_placeholder_window(&mut self) {
       ...
       /*todo  let new_window = Frame::new("Placeholder".to_string(), 0,true);
        let mut state = self.state.lock().unwrap();
        let window_index = state.windows.len();
        state.windows.push(new_window);
        state.grid.push(window_index);*/
    ...
    pub fn minimize_window(&mut self, index: usize) {
       //TODO: self.windows[index].is_minimized = !self.windows[index].is_minimized;
    }
